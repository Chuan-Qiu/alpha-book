...
function ReturnChart({ data }: { data: { x: number; y: number; label: string }[] }) {
  ...
  return (
    <View className="mx-4 mb-4 bg-surface rounded-2xl p-3"
      ...
      <Text className="text-gray-500 text-xs mb-2">累计收益率走势</Text>
    ...
    <View className="bg-surface rounded-2xl p-4 mb-3 mx-4"
      ...
      <View className="flex-row justify-between items-start mb-3">
        <View className="flex-1">
          <Text className="text-gray-900 font-semibold text-base">{trade.stock_name}</Text>
          <Text className="text-gray-400 text-xs">{trade.stock_code}</Text>
        </View>
      ...
      {/* 买入/卖出汇总 */}
      <View className="flex-row pt-3 gap-4" style={{ borderTopWidth: 1, borderTopColor: "#F3F4F6" }}>
        <View className="flex-1">
        ...
        {!open ? (
          ...
          <View className="items-end justify-center">
            <View className="bg-amber-50 px-2 py-1 rounded-lg">
              <Text className="text-amber-600 text-xs font-medium">持仓中</Text>
            </View>
          </View>
        )}
      </View>
      ...
      {trade.notes && (
        <Text className="text-gray-400 text-xs mt-2 italic">{trade.notes}</Text>
      )}
      ...
      <View className="mx-4 mb-4 bg-surface rounded-2xl p-4"
        ...
        <View className="flex-row justify-around">
          <View className="items-center">
            <Text className="text-gray-500 text-xs mb-1">总收益率</Text>
          ...
          <View className="items-center">
            <Text className="text-gray-500 text-xs mb-1">交易笔数</Text>
            <Text className="text-gray-900 text-2xl font-bold">{trades.length}</Text>
          </View>
          <View className="items-center">
            <Text className="text-gray-500 text-xs mb-1">胜率</Text>
